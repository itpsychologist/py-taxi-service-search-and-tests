...
class ManufacturerViewsTestCase(TestCase):
    def setUp(self):
        ...
        self.manufacturer = Manufacturer.objects.create(name="BMW Group", country="Germany")
    ...
    def test_manufacturer_update_view(self):
        response = self.client.post(reverse("taxi:manufacturer-update", args=[self.manufacturer.id]), {
            "name": "BMW Updated",
...
class CarListViewTestCase(TestCase):
    def setUp(self):
        ...
        # Create test cars
        self.car1 = Car.objects.create(model="Toyota Camry", manufacturer=self.manufacturer)
        self.car2 = Car.objects.create(model="Honda Accord", manufacturer=self.manufacturer)
        self.car3 = Car.objects.create(model="Ford Focus", manufacturer=self.manufacturer)
        self.car4 = Car.objects.create(model="Toyota Corolla", manufacturer=self.manufacturer)
        self.car5 = Car.objects.create(model="Honda Civic", manufacturer=self.manufacturer)
        self.car6 = Car.objects.create(model="Nissan Altima", manufacturer=self.manufacturer)
    ...
    def test_car_list_view_filtering(self):
        ...
        response = self.client.get(reverse("taxi:car-list"), {"model": "Honda"})
        self.assertEqual(len(response.context["object_list"]), 2)  
    ...
    def test_car_list_view_redirects_when_not_logged_in(self):
        ...
        response = self.client.get(reverse("taxi:car-list"))
        self.assertEqual(response.status_code, 302)  
        self.assertRedirects(response, f"/accounts/login/?next={reverse("taxi:car-list")}")
